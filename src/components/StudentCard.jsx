import React from 'react'
const StudentCard = (props) => {
  return (
    <div className="col-md-4 mb-4">
      <div className="card student-card shadow-sm">
        <div className="card-body">
          <p className="card-title">Student id:{props.id}</p>
          <h5 className="card-title">{props.name}</h5>
          <p className="card-text">Course: {props.course}</p>
          <p className="card-text">Batch: {props.batch}</p>
          <button className="btn btn-primary btn-sm me-2" onClick={props.onView} >
            View
          </button>
          <button className="btn btn-success btn-sm me-2" onClick={props.onUpdate}>
            Update
          </button>
          <button className="btn btn-danger btn-sm" onClick={props.onDelete}>
            Delete
          </button>
        </div>
      </div>
    </div>
  )
}

export default StudentCard
